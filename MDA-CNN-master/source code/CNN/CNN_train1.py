...
def get_samples(args):
    train_data = pd.read_csv(args.input_disease_miRNA)
...
def parse_args():
    ...
    parser.add_argument('--input_disease_miRNA', nargs='?', default='/Users/qiangdanlei/Desktop/graduate thesis/MDA-CNN-master/data/CNN/disease_miRNA.csv',
                        help='Input disease_gene_relationship file')

    parser.add_argument('--input_label',nargs = '?',default='/Users/qiangdanlei/Desktop/graduate thesis/MDA-CNN-master/data/CNN/label1.csv',
                        help='sample label')
    parser.add_argument('--batch_size', nargs='?', default=128,
                        help = 'number of samples in one batch')
    parser.add_argument('--training_epochs', nargs='?', default=200 ,
                        help= 'number of epochs in SGD')
    parser.add_argument('--display_step', nargs='?', default=50)
...
def max_pool_2(x, W):
    return tf.nn.max_pool2d(x, ksize = W, strides= [1,10,1,1], padding= "VALID")


def get_data(args):

    input_data, input_label = get_samples(args)
        ...
        with tf.compat.v1.Session() as sess:
                ...
                for i, batch in enumerate(batches):
                    ...
                    if i % display_step == 0:
                      ...
                      #  print('train accuracy='+str(accuracy.eval(feed_dict={input_data: x_train, input_label: y_train, keep_prob: 1.0})))
                       ...
                       # print('cal accuracy=' + str(accuracy.eval(feed_dict={input_data: x_dev, input_label: y_dev, keep_prob: 1.0})))
                        # print('test loss = %f' % loss)

                        false_positive_rate1, true_positive_rate1, thresholds1 = roc_curve(np.array(y_dev)[:, 1], y_predict)
                        roc_auc1 = auc(false_positive_rate1, true_positive_rate1)
                        #print(roc_auc1)
                        print("prediction: ",accuracy.eval(feed_dict = {input_data: test_data, input_label: test_label, keep_prob :1.0}))
                ...
                y_predict = sess.run(y_res, feed_dict={input_data: test_data, input_label: test_label, keep_prob: 1.0})[
                            :, 1]
                false_positive_rate1, true_positive_rate1, thresholds1 = roc_curve(np.array(test_label)[:, 1],
                                                                                   y_predict)
                roc_auc1 = auc(false_positive_rate1, true_positive_rate1)
               # plt.plot(false_positive_rate1, true_positive_rate1)
                plt.plot(false_positive_rate1, true_positive_rate1, color='darkorange',
                          label='ROC curve (area = %0.2f)' % roc_auc1)  ###假正率为横坐标，真正率为纵坐标做曲线
                ...
                plt.plot(precision, recall,label='P-R curve (area = %0.2f)'%(average_precision_score(np.array(test_label)[:, 1], y_predict)))
                plt.plot([0, 1], [0, 1], color='navy', linestyle='--')
                plt.xlim([0.0, 1.0])
                plt.ylim([0.0, 1.0])
                ...
                #print('Average  score: {0:0.2f}'.format(average_precision))
                y_predict[y_predict >= 0.5] = 1
                y_predict[y_predict < 0.5] = 0
                #print(y_predict)
                print("f1-score",metrics.f1_score(np.array(test_label)[:, 1], y_predict))
                print("recall-score", metrics.recall_score(np.array(test_label)[:, 1], y_predict))
                print("precsion-score", metrics.precision_score(np.array(test_label)[:, 1], y_predict))
